...
public:
	explicit Helper(const net::QuicClock *clock, quux::IsaacRandom* quic_random,
			...
			QuicConnectionHelperInterface(), clock(clock), quic_random(
					quic_random), buffer_allocator(buffer_allocator) {
	}
...
class Logger: public net::QuicConnectionDebugVisitor {
	...
	virtual void OnPacketSent(const net::SerializedPacket& serialized_packet,
			...
			net::TransmissionType transmission_type, net::QuicTime sent_time) {
		printf("[LOG] %s\n", __FUNCTION__);
	}
	...
	virtual void OnPacketReceived(const net::IPEndPoint& self_address,
			...
			const net::QuicEncryptedPacket& packet) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when the unauthenticated portion of the header has been parsed.
	virtual void OnUnauthenticatedHeader(const net::QuicPacketHeader& header) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when a packet is received with a connection id that does not
	// match the ID of this connection.
	virtual void OnIncorrectConnectionId(net::QuicConnectionId connection_id) {
		printf("[LOG] %s\n", __FUNCTION__);
	}
	...
	virtual void OnDuplicatePacket(net::QuicPacketNumber packet_number) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when the protocol version on the received packet doensn't match
	// current protocol version of the connection.
	virtual void OnProtocolVersionMismatch(net::QuicVersion version) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when the complete header of a packet has been parsed.
	virtual void OnPacketHeader(const net::QuicPacketHeader& header) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when a StreamFrame has been parsed.
	virtual void OnStreamFrame(const net::QuicStreamFrame& frame) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when a AckFrame has been parsed.
	virtual void OnAckFrame(const net::QuicAckFrame& frame) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when a StopWaitingFrame has been parsed.
	virtual void OnStopWaitingFrame(const net::QuicStopWaitingFrame& frame) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when a net::QuicPaddingFrame has been parsed.
	virtual void OnPaddingFrame(const net::QuicPaddingFrame& frame) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when a Ping has been parsed.
	virtual void OnPingFrame(const net::QuicPingFrame& frame) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when a GoAway has been parsed.
	virtual void OnGoAwayFrame(const net::QuicGoAwayFrame& frame) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when a RstStreamFrame has been parsed.
	virtual void OnRstStreamFrame(const net::QuicRstStreamFrame& frame) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when a ConnectionCloseFrame has been parsed.
	virtual void OnConnectionCloseFrame(
			const net::QuicConnectionCloseFrame& frame) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when a WindowUpdate has been parsed.
	virtual void OnWindowUpdateFrame(const net::QuicWindowUpdateFrame& frame) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when a BlockedFrame has been parsed.
	virtual void OnBlockedFrame(const net::QuicBlockedFrame& frame) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when a PathCloseFrame has been parsed.
	virtual void OnPathCloseFrame(const net::QuicPathCloseFrame& frame) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when a public reset packet has been received.
	virtual void OnPublicResetPacket(const net::QuicPublicResetPacket& packet) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when a version negotiation packet has been received.
	virtual void OnVersionNegotiationPacket(
			const net::QuicVersionNegotiationPacket& packet) {
		printf("[LOG] %s\n", __FUNCTION__);
	}
	...
	// |config|.
	virtual void OnSetFromConfig(const net::QuicConfig& config) {
		printf("[LOG] %s\n", __FUNCTION__);
	}

	// Called when RTT may have changed, including when an RTT is read from
	// the config.
	virtual void OnRttChanged(net::QuicTime::Delta rtt) const {
		printf("[LOG] %s\n", __FUNCTION__);
	}
};
